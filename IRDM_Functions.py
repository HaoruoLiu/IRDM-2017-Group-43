...
### find number of digit in query(str1) appear in test(srt2)
...
### find the ratio of two str' sequence
...
### deletes all words contain digits
...
def str_stemmer(s, automatic_spell_check_dict={},remove_from_brackets=False,parse_material=False,add_space_stop_list=[], stoplist=stoplist):
    s=str_parser(s,automatic_spell_check_dict=automatic_spell_check_dict, remove_from_brackets=remove_from_brackets,            parse_material=parse_material, add_space_stop_list=add_space_stop_list)
...
def col_parser(clmn, automatic_spell_check_dict={}, remove_from_brackets=False,parse_material=False,add_space_stop_list=[]):
    ...
    for i in range(0,len(aa)):
        my_dict[aa[i]]=str_parser(aa[i],automatic_spell_check_dict=automatic_spell_check_dict, remove_from_brackets=remove_from_brackets,                                    parse_material=parse_material,add_space_stop_list=add_space_stop_list)
        if (i % 10000)==0:
...
def find_similarity(w1,w2, nouns=True,CUT_VALUE=14.50):
    ...
    similarities_list=[item1.path_similarity(item2)  for item1 in lst1                                 for  item2 in lst2                                 if item1.path_similarity(item2)!=None]                     
    ...
    lch_similarities_list=[item1.lch_similarity(item2)  for item1 in lst1                                 for  item2 in lst2                                 if item1.pos()==item2.pos() and item1.lch_similarity(item2)!=None]                     
    ...
    res_similarities_list=[min(CUT_VALUE,item1.res_similarity(item2,brown_ic))  for item1 in lst1                                 for  item2 in lst2                                 if item1.pos() not in ['a','s','r'] and item1.pos()==item2.pos() and item1.res_similarity(item2,brown_ic)!=None]  
